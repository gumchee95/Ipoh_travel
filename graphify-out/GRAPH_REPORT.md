...
- `js/csvLoader.js`: generated JSON loading.
...
4. Prefer `places.js`, `script.js`, `style.css`, and `index.html` over broad scans.
...
- `data/json/` is generated by `scripts/build-data.js`.
...
- Router audit found no original `routing_result.json` or activation log from project startup.
- Current routing state is reconstructed in `01_Work/routing_result.json`.
